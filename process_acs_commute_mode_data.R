...
process_pums_data <- function(years, pums_span=1) {
  ...
  pums_vars_old <- c("JWTR", "JWRIP", "WAGP", "PRACE", "RAC1P", "RAC2P", "ANC1P")
  pums_vars_new <- c("JWTRNS", "JWRIP", "WAGP", "PRACE", "RAC1P", "RAC2P", "ANC1P")
  ...
  for (i in years) {
    ...
    p <- pums |> 
      dplyr::rename(mode = tidyselect::starts_with("JWTR"), occupancy = "JWRIP", wage = "WAGP") |> 
      ...
      dplyr::mutate(race_6cat = factor(dplyr::case_when(is.na(.data$PRACE) ~ NA_character_,
      ...
      dplyr::mutate(race_3cat =  factor(case_when(.data$PRACE %in% c("Asian alone","Native Hawaiian and Other Pacific Islander alone") ~ "Asian or Pacific Islander",
                                                  .data$PRACE == "White alone" ~ "White",
                                                  TRUE ~ "Other people of color"),
                                        levels = c("Asian or Pacific Islander","Other people of color","White"))) |>
      
      dplyr::mutate(race_5cat =  factor(case_when(.data$PRACE %in% c("Asian alone","Native Hawaiian and Other Pacific Islander alone") ~ "Asian or Pacific Islander",
                                                  .data$PRACE == "White alone" ~ "White",
                                                  .data$PRACE == "Hispanic or Latino" ~ "Hispanic or Latinx",
                                                  .data$PRACE == "Black or African American alone" ~ "Black or African American",
      ...
      dplyr::mutate(country =  factor(case_when(
      ...
      dplyr::mutate(aapi_detailed =  factor(case_when(
    ...
    aapi_top_10 <- p |>
      ...
      dplyr::filter(!.data$aapi_detailed %in% c("Total","All combinations of Asian races only", "Other Asian alone")) |>
      dplyr::arrange(desc(.data$count)) |>
      dplyr::slice_head(n=10) |>
      dplyr::select("aapi_detailed") |>
      dplyr::pull()
    ...
    print(stringr::str_glue("Calculating Commute Mode by AAPI 5-Category Race for PUMS {pums_span}-year data for {i}"))
    modes_race <- psrccensus::psrc_pums_count(p, group_vars = c("race_5cat", "mode"), incl_na = FALSE) |>
      dplyr::rename(race="race_5cat") |>
    ...
    print(stringr::str_glue("Calculating Commute Mode by Detailed Asian Ancestry for PUMS {pums_span}-year data for {i}"))
    modes_asian <- p |> 
      dplyr::filter(.data$race_3cat == "Asian or Pacific Islander" & .data$aapi_detailed %in% aapi_top_10) |> 
      psrccensus::psrc_pums_count(p, group_vars = c("race_3cat","aapi_detailed", "mode"), incl_na = FALSE) |>
      filter(!.data$aapi_detailed %in% c("Total","All combinations of Asian races only", "Other Asian alone")) |>
      dplyr::rename(race="aapi_detailed") |>
      dplyr::filter(.data$mode != "Total") |>
      dplyr::select(-"share_moe") |>
      dplyr::select(geography = "race", variable = "mode", year = "DATA_YEAR", estimate = "count", "share", moe = dplyr::ends_with("moe")) |>
      dplyr::mutate(metric = "Commute Mode by Asian Ancestry", year = as.character(.data$year)) |>
      dplyr::select("year", "geography", "variable", "metric", "estimate", "share", "moe")
    
    # Combine summarized tables
    ifelse(is.null(processed), processed <- dplyr::bind_rows(mean_wage_mode, modes_race, modes_asian), processed <- dplyr::bind_rows(processed, mean_wage_mode, modes_race, modes_asian))
